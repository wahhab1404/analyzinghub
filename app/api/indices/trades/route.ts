...
// Give the create route headroom for the (now non-blocking) external calls it
// still awaits — Polygon snapshots, the re-entry RPC and the insert — so it is
// never cut off by the platform's default ~10s function timeout.
export const maxDuration = 60;

/**
 * Generate the trade snapshot image and persist its URL to
 * index_trades.contract_url. Runs DETACHED (fire-and-forget) from the create
 * request: the headless render can take several seconds and, when awaited
 * inline, used to push the POST past the serverless gateway timeout. The
 * browser then saw a 504/aborted fetch and showed "Failed to add trade" even
 * though the trade had already been inserted and its Telegram announcement
 * queued. The telegram-outbox-processor renders the image natively at send
 * time (and re-reads contract_url from the DB), so a missed/aborted render
 * here is harmless. Never throws.
 */
async function generateTradeSnapshotInBackground(tradeId: string): Promise<void> {
  ...
  if (!supabaseUrlEnv || !serviceRoleKey) return;

  console.log(`[trade-create] 🖼  Generating snapshot for trade ${tradeId} via generate-trade-snapshot edge function (background)...`);
  try {
    const snapshotResponse = await fetch(`${supabaseUrlEnv}/functions/v1/generate-trade-snapshot`, {
      ...
      body: JSON.stringify({ tradeId, isNewHigh: false, appBaseUrl }),
    });
    ...
    const snapshotUrl = result.imageUrl || null;
    if (!snapshotUrl) return;
    console.log(`[trade-create] ✅ Snapshot generated: ${snapshotUrl}`);

    const admin = createClient(supabaseUrlEnv, serviceRoleKey);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // processor trigger inside is fire-and-forget). The outbox processor renders
    // the announcement image natively at send time and re-reads contract_url
    // from the DB, so it does not need a pre-generated snapshot URL here.
    await publishNewTradeToTelegram(trade, body, trade.contract_url ?? null);

    // ── Snapshot generation (best-effort, NON-BLOCKING) ──────────────────────────
    // Fired detached so a slow headless render never delays the create response.
    // Awaiting it inline previously pushed the request past the serverless
    // gateway timeout, which the UI misreported as "Failed to add trade" even
    // though the trade was already created and announced. See
    // generateTradeSnapshotInBackground.
    void generateTradeSnapshotInBackground(trade.id);
