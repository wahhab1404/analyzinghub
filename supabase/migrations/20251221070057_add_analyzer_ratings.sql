/*
  ...
  3. Constraints
    ...
    - Can only rate users with analyzer role
  
  4. Functions
    - Trigger to update rating stats when ratings change
    - Function to calculate and update aggregate stats
*/
...
CREATE POLICY "Users can create own ratings"
  ...
  TO authenticated
  WITH CHECK (auth.uid() = user_id AND analyzer_id != user_id);
