/*
  # Fix re-entry / finalization outcome value

  ## Problem
  `process_trade_new_entry` and `finalize_trade_canonical` set the text
  `outcome` column to 'win' for a winning trade (max_profit >= 100). The
  `valid_outcome` CHECK constraint on index_trades only permits
  NULL / 'succeed' / 'loss' / 'expired', so re-entering (NEW_ENTRY) or
  finalizing a *winning* contract failed with:

    new row for relation "index_trades" violates check constraint "valid_outcome"

  ## Fix
  Use the canonical 'succeed' value (matching existing data and the
  constraint) for the text `outcome` column. The `trade_outcome` enum and
  `is_win` boolean are unchanged in meaning. No data backfill is needed —
  no row with the invalid value could ever have been committed.
*/
...
CREATE OR REPLACE FUNCTION public.finalize_trade_canonical(p_trade_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
