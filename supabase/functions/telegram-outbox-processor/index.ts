...
async function processTradeMessage(
  ...
  // Resolve image bytes — try Storage URL first (already generated by
  // generate-trade-snapshot), then fall back to calling the Next.js API.
  if (trade?.id) {
    let imgBytes: ArrayBuffer | null = null;

    // 1. Try Supabase Storage URL (contract_url set by generate-trade-snapshot)
    if (trade?.contract_url && !trade.contract_url.includes('localhost')) {
      ...
      console.log(`[outbox:processTradeMessage] ℹ️  No contract_url in trade — skipping Storage step`);
    }

    // 2. Fall back to generating fresh from Next.js generate-image API
    if (!imgBytes) {
