...
async function processTradeMessage(
  ...
  // Re-fetch contract_url from DB — the payload may be captured before
  // generate-trade-snapshot sets it.
  if (trade?.id) {
  ...
  console.log(`[outbox:processTradeMessage]   contractUrl:  ${trade?.contract_url ?? 'none'}`);
  ...
  // Telegram's servers will download the image from Supabase Storage directly.
  // Works because the chart-images bucket is public.
  if (hasStorageUrl) {
