...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const avgProfit = trades && trades.length > 0
      ? trades.reduce((sum, t) => sum + (t.max_profit_percent || 0), 0) / trades.length
      : 0;

    const maxProfit = trades && trades.length > 0
      ? Math.max(...trades.map(t => t.max_profit_percent || 0))
      : 0;

    const winningTrades = trades?.filter(t => (t.max_profit_percent || 0) > 0).length || 0;
    ...
    // For now, we'll return the HTML and convert to PDF on the client side
    ...
    <div class="trades-section">
      ...
      ${trades.length === 0 ? `
        <div class="no-trades">
          ...
          <h3>No Trades Today</h3>
          <p>There were no trades recorded for today.</p>
        </div>
            ...
            <div class="trade-details">
              ...
              ${trade.status === 'closed' ? `
                <div class="trade-detail">
                  ...
                  <div class="trade-detail-value" style="font-size: 14px;">${trade.exit_reason || 'Manual Close'}</div>
                </div>
              ` : ''}
            </div>
          </div>
...
async function sendToTelegramChannels(html: string, supabase: any, stats: any) {
  try {
    ...
    // Get all active analyzer plans with Telegram channels
...
<i>Generated by AnalyZHub - Professional Trading Platform</i>
      `.trim();
      ...
      try {
        ...
        console.log(`Daily report sent to channel: ${plan.telegram_channel_username}`);
      } catch (err) {
        console.error(`Error sending to channel ${plan.telegram_channel_username}:`, err);
      }
    }
  } catch (error) {
